...
async function refreshMatchWithDetails(matchId: number) {
  ...
  const matchWeek: number | null = Number.isNaN(matchWeekParsed) ? null : matchWeekParsed;
  const fallbackVenueId = venueId ?? homeTeam.homeVenueId ?? null;
  const matchData = {
...
export async function GET(
  ...
  try {
    ...
    if (!match) {
      try {
        match = await refreshMatchWithDetails(matchId);
      } catch (saveError) {
    ...
    if (match) {
      ...
      if (hasMissingDetails) {
        try {
          const refreshedMatch = await refreshMatchWithDetails(matchId);
          if (refreshedMatch) match = refreshedMatch;
        } catch (saveError) {
          console.error('Error refreshing incomplete match data:', saveError);
        }
      }
    }
